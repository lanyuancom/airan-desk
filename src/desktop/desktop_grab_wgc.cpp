...
struct DesktopGrabWGC::Impl
{
    ...
    {
        std::weak_ptr<Impl> weakImpl;
        void operator()(Direct3D11CaptureFramePool const &sender,
...
void DesktopGrabWGC::Impl::onFrameArrived(Direct3D11CaptureFramePool const &sender)
{
    if (isStopping.load())
    {
        return;
    ...
    try
    {
        ...
        {
            if (contentSize.Width != lastFramePoolSize.Width ||
                contentSize.Height != lastFramePoolSize.Height)
            {
                sender.Recreate(
                    winrtDevice,
...

        {
            ...
            bool needRecreate = !latestTexture;
            if (!needRecreate)
            {
                D3D11_TEXTURE2D_DESC curDesc{};
                latestTexture->GetDesc(&curDesc);
            ...
            if (needRecreate)
            {
                ...
                latestTexture.Reset();
                HRESULT chr = d3d11Device->CreateTexture2D(&copyDesc, nullptr, &latestTexture);
                if (FAILED(chr) || !latestTexture)
                {
                    return;
                }
            }

            d3d11Context->CopyResource(latestTexture.Get(), srcTexture.Get());
            hasNewFrame = true;
        }
    }
    catch (...)
...
void DesktopGrabWGC::Impl::FrameArrivedHandler::operator()(
...
bool DesktopGrabWGC::init(int screenIndex)
{
    ...
    try
    {
        ...
        impl_->frameArrivedToken = impl_->framePool.FrameArrived(
            Impl::FrameArrivedHandler{impl_});
