...
public:
    ...
    bool subscribe(const QString &subscriberId, int screenIndex, int dstW, int dstH, int fps,
                   int targetKbps = 0, bool forceAllKeyframes = false);
    bool setCaptureBackend(const QString &backend, int screenIndex);
    bool requestKeyframe(const QString &subscriberId, int screenIndex);
    QStringList availableCaptureBackends() const;
...
public slots:
    ...
    void onWorkerCaptureBackendChanged(int screenIndex, const QString &requestedBackend, const QString &actualBackend);
    void onWorkerEncoderChanged(int screenIndex, const QString &encoderName, bool isHardware, bool zeroCopy);
...
private:
    ...
    QMap<int, QString> m_requestedBackends;      /* key: screenIndex */

    QMap<QString, int> m_subscriberScreens;
};
